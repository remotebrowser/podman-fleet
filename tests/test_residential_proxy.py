...
from podmanfleet.residential_proxy import (
    GeoLocation,
    MassiveProxyConfig,
    OxylabsProxyConfig,
    get_proxy_config,
...
async def test_get_proxy_config_no_location_returns_none() -> None:
    ...
    with patch("podmanfleet.residential_proxy.get_location", new=AsyncMock(return_value=None)):
        result = await get_proxy_config("1.2.3.4", s)
    assert result is None


@pytest.mark.asyncio
async def test_get_proxy_config_default_selection() -> None:
    s = _settings(
    ...
    loc = _us_location()
    with patch("podmanfleet.residential_proxy.get_location", new=AsyncMock(return_value=loc)):
        result = await get_proxy_config("1.2.3.4", s)
